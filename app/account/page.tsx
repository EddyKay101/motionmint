...
export default function AccountPage() {
  ...
  return <main className="account-page"><div className="account-card"><Link className="auth-brand dark" href="/">Turnbine<i /></Link><p className="auth-eyebrow">Your account</p><div className="account-avatar">{data.user.name?.slice(0, 1).toUpperCase() || "T"}</div><h1>{data.user.name}</h1><p>{data.user.email}</p><div className="account-actions"><Link href="/create">Open creator →</Link><button onClick={() => void authClient.signOut({ fetchOptions: { onSuccess: () => { router.push("/"); router.refresh(); } } })}>Sign out</button></div></div></main>;
}
