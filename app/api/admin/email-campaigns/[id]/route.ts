...
import { compileCampaignHtml, type EmailCampaignConfig } from "../../../../../lib/email-builder";
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updates: Partial<typeof emailCampaigns.$inferInsert> = { updatedAt: new Date().toISOString() };
    ...
    if (payload.preheader !== undefined) updates.preheader = payload.preheader?.trim().slice(0, 200) || null;
    ...
    await getDb().update(emailCampaigns).set(updates).where(eq(emailCampaigns.id, id));
    return Response.json({ campaign: { id, ...updates, configJson: undefined, htmlCache: undefined } });
