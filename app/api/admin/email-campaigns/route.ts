...
export async function GET(request: Request) {
  ...
  try {
    const rows = await getDb().select().from(emailCampaigns).orderBy(asc(emailCampaigns.name));
    return Response.json({
      campaigns: rows.map(({ configJson, htmlCache, ...row }) => ({ ...row, config: JSON.parse(configJson) })),
    });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await getDb().insert(emailCampaigns).values({
      id,
    ...
    return Response.json({ campaign: { id, status: "draft" } }, { status: 201 });
  } catch (error) { return databaseError(error); }
}
