...
import { generateEmailConcepts, type EmailStudioBrief } from "../../../../lib/email-studio";
import { generateOpenAIEmailConcepts } from "../../../../lib/openai-email-studio";
import { env } from "cloudflare:workers";

export async function POST(request: Request) {
  if (!await canManageTemplates(request)) return apiError("Email Studio is restricted to administrators.", 403);
  try {
    ...
    if (apiKey && !apiKey.includes("replace_with")) {
      ...
      return Response.json({ provider: model, simulated: false, concepts, note: `Original drafts generated with ${model}. Review before sending.` });
    }
    ...
    return apiError(error instanceof Error ? error.message : "Could not generate email concepts.");
  }
}
