...
const slug = (value: string) => value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "motionmint-email";
const download = (name: string, content: string, type: string) => {
  const url = URL.createObjectURL(new Blob([content], { type }));
  const link = document.createElement("a"); link.href = url; link.download = name; link.click(); URL.revokeObjectURL(url);
};
const ownerStorageKey = "motionmint.owner.v1";
const projectStorageKey = "motionmint.project.v1";
const getOwnerKey = () => {
  let key = localStorage.getItem(ownerStorageKey);
  if (!key) { key = crypto.randomUUID().replaceAll("-", "") + crypto.randomUUID().replaceAll("-", ""); localStorage.setItem(ownerStorageKey, key); }
  return key;
};
const bannerSnapshot = (project: BannerProject) => {
  ...
  const pad = width * .075, scene = project.scenes?.[0]; context.fillStyle = accent; context.font = `700 ${Math.max(15, width * .018)}px Arial`; context.fillText((project.category || "MOTIONMINT").toUpperCase(), pad, height * .13);
...
export function EmailCreator({ templateId }: { templateId: string }) {
  ...
  const storageKey = `motionmint.email.${templateId}`;
  ...
  const loadBannerProjects = async () => {
    setCreativeOpen(true); setStatus("Loading your MotionMint creatives…");
    let local: BannerProject | undefined;
    try { const raw = localStorage.getItem(projectStorageKey); if (raw) local = JSON.parse(raw) as BannerProject; } catch { /* ignore damaged local project */ }
    try {
      const response = await fetch("/api/projects", { headers: { "x-motionmint-owner": getOwnerKey() }, cache: "no-store" });
  ...
  const insertBannerProject = (project: BannerProject) => {
    try {
      const block: EmailBlock = { type: "image", src: bannerSnapshot(project), alt: `${project.title} MotionMint creative`, href: creativeHref.trim() || undefined };
  ...
  if (!draft) return <main className="email-creator-loading"><Link href="/">Motion<span>Mint</span></Link><p>{status}</p><Link href="/email">← Back to email templates</Link></main>;
      ...
      <section className="email-creator-workspace">
        <div className="email-creator-controls">
          ...
          <fieldset>
            ...
            <div className="email-motionmint-insert">
              <div><b>Insert a MotionMint creative</b><p>Reuse one of your saved banner projects as a clickable, email-safe visual.</p></div>
