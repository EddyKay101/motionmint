...
const contains = (source: string, words: string[]) =>
  words.some((word) => source.includes(word));

function inferGoal(prompt: string, requested?: string) {
  if (requested?.trim()) return requested.trim();
  const text = prompt.toLowerCase();
  if (contains(text, ["launch", "new feature", "release"]))
    return "Announce a launch";
  if (contains(text, ["sale", "discount", "offer", "promo"]))
    return "Drive a promotion";
  if (contains(text, ["event", "webinar", "workshop"]))
    return "Invite to an event";
  if (contains(text, ["newsletter", "update", "digest"]))
...
const openers: Record<string, string[]> = {
  ...
  "Drive a promotion": [
    "Don't miss this.",
    "A deal worth opening.",
    "Limited time, real value.",
  ],
...
function buildConcept(
  ...
  const subjectBase =
    titleCase(
      prompt
        .replace(
          /\b(write|create|draft|email|campaign|about|for|the|and|an|a)\b/gi,
          " ",
        )
        .trim(),
    ) || goal;
  ...
  const blocks: EmailBlock[] = [
    ...
    {
      type: "paragraph",
      text: `${subjectBase}. Crafted with ${tone.toLowerCase()} tone to match your brand voice.`,
    },
    { type: "divider" },
    {
      type: "paragraph",
      text: "Tell your audience why this matters right now and what happens when they click through.",
    },
  ...
  return {
    ...
    subject: subjectBase,
    preheader: `${openerSet[index % openerSet.length]} ${subjectBase}`.slice(
      0,
      90,
    ),
    config,
...
export function generateEmailConcepts(
  brief: EmailStudioBrief,
): EmailStudioConcept[] {
  const prompt = brief.prompt.trim().slice(0, 2000);
  if (prompt.length < 12)
    throw new Error("Describe the email campaign in at least 12 characters.");
  ...
  return Array.from({ length: 3 }, (_, index) =>
    buildConcept(prompt, goal, tone, seed, index),
  );
}
